...
{return formatDate(date||new Date(),format);},pad:pad,rpad:rpad};}(jQuery));(function($){var mode={clock:'clock',explicit:'explicit',countdown:'countdown',countup:'countup',rollover:'rollover',expire:'expire',loop:'loop',stopwatch:'stopwatch',holdup:'holdup',timer:'timer'},_=function(selector)
...
{case mode.holdup:case mode.rollover:clock.__displacement=0;clock.mode=mode.countup;clock.time=now.valueOf();return zero;case mode.expire:case mode.countdown:case mode.timer:clock.destroy();return zero;case mode.loop:clock.time=now.valueOf();return zero;}}
function tick(clock)
...
return zero;case mode.countup:current-=clock.time;break;case mode.explicit:current+=clock.time;break;case mode.rollover:case mode.loop:case mode.expire:case mode.countdown:case mode.timer:current=clock.time-current;if(current<1e3)
{return terminate(clock,current);}
break;}
...
break;case mode.stopwatch:clock.__displacement=clock.__offset+(-1*new Date().valueOf());clock.__tare=true;return clock;default:throw'EPICLOCK_INVALID_MODE';}
...
{this.clock.destroy();});}});$.dateformat.rules({Q:function()
...
{this.__destroy=true;triggerEvent(this.__uid,'destroy');},print:function(format)
